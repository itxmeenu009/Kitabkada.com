...
const grid = document.getElementById('books-grid-en');

async function loadBooks() {
    try {
        const q = query(collection(db, "books"), where("category", "==", "english"));
        const snapshot = await getDocs(q);
        grid.innerHTML = '';

        if (snapshot.empty) {
            grid.innerHTML = '<p id="loading-spinner">No English books found yet.</p>';
            return;
        }
